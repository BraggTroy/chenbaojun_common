...
ES.util.login = function (hasBack) {
	if (ES.sso_truck.loginType === "sso") {
		...
		hasBack ? ES.util.forwardredirect('login.html') : ES.util.redirect('login.html')
	}
}
...
ES.util.authority_redirect = function (authorities) {
	ES.each(ES.consts.authorities, function (_, redirectUrl) {
		...
		if (result) {
			...
			if(authorities.indexOf("BUYER_FLEET_CONTAINER_ORDER_LIST") >= 0){
				ES.util.backdirect(ES.msg.get("BUYER_FLEET_CONTAINER_ORDER_LIST"))
			}else {
	...
	img.onload = function () {
		...
		var handleScroll = function (evt) {
			...
			if (evt.type == 'click') {
				...
				if (ES.get(evt.toElement).hasClass('fa-search-minus')) {
					delta = -3
				} else if (ES.get(evt.toElement).hasClass('fa-search-plus')) {
					delta = 3
				}
			} else {
